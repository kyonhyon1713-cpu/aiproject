...
//
// Connect to WiFi "bkh" (set your local AP password below), open http://192.168.4.1
#include <EI_project_inferencing.h>
#include <WiFi.h>
#include <WebServer.h>
...
#result{font-size:22px;font-weight:bold;margin:10px 0 6px}
#lat{color:#888;font-size:13px;margin-top:8px}
</style></head><body>
<h2>🔍 배터리 / 선풍기 실시간 추론</h2>
<div id="wrap"><img id="cam" src="/jpg"><div id="box"></div><div id="tag">...</div></div>
<div id="result">추론 준비 중...</div>
<div id="bars"></div>
<div id="lat"></div>
<script>
const COLORS={battery:'#4cf58b',fan:'#f5c94c'};
...
async function tick(){
  try{
    ...
    document.getElementById('result').textContent=`추론: ${d.label} (${(d.confidence*100).toFixed(1)}%)`;
